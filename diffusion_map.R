...
dpt <- DPT(dif)
plot(dpt, root = 2,pal = coul)
plot(dif, 1:2,col_by = 'subtype',pch = 20)

rd2 <- cbind(DC1 = dif$DC1, DC2 = dif$DC2)
reducedDims(object.sce) <- SimpleList(DiffMap = rd2)
object.sce <- slingshot(object.sce, clusterLabels = Idents(seu.destiny), reducedDim = 'DiffMap')
plot(reducedDims(object.sce)$DiffMap, col = pal[Idents(seu.destiny)], pch=16, asp = 1)
lines(SlingshotDataSet(object.sce), lwd=3, col='black')

plot(reducedDims(object.sce)$DiffMap, col = pal[Idents(seu.destiny)], pch=16, asp = 1)
lines(SlingshotDataSet(object.sce), lwd=3, type = 'lineages', col = 'black')

#plot 3D
rd3 <- cbind(coord1 = dif$coord1, coord2 = dif$coord2, coord3 = dif$coord3)
reducedDims(object.sce) <- SimpleList(DiffMap3D = rd3)
test <- slingshot(reducedDims(object.sce)$DiffMap3D, clusterLabels = Idents(seu.destiny))
plot3d(reducedDims(object.sce)$DiffMap3D, col = pal[Idents(seu.destiny)], aspect = 'iso', alpha = 0.3)
plot3d(test, lwd = 3, add = TRUE)
