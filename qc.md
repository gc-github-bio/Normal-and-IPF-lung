...
#Perform qc analysis on each dataset separately
...
```r
#filter out AT2 (SFTPC+), immune (PTPRC+), and endothelial (PECAM1+) cells
...
VlnPlot(object=seu,c("SFTPC","PTPRC","PECAM1"),pt.size = 0)
```

![](qc_files/figure-html/unnamed-chunk-2-4.png)<!-- -->

```r
seu <- subset(x = seu, idents = c("0","3","8"), invert = TRUE)                                                                                       
saveRDS(seu,'/Volumes/GC_2T/qc/dataset.rds')
```
